...
{   
    ...
    for(int i = 0; i < numTurbines; i++)
    {
        ...
        bladeInfluenceCells.append(DynamicList<label>);
        nacelleInfluenceCells.append(DynamicList<label>);
        towerInfluenceCells.append(DynamicList<label>);
    }
...
void horizontalAxisWindTurbinesALMAdvanced::findRotorSearchCells(int turbineNumber);
{
    ...
    // option below.
    if (bladeSearchCellMethod == "sphere")
    {
    ...
    else if (bladeSearchCellMethod == "disk")
    {
        forAll(U_.mesh().cells(),cellI)
        {
            ...
            scalar r = 0.0;
            scalar xBlade = 0.0;
            v = U_.mesh().cells()[cellI] - rotorApex[i];
            zP.z() = 1.0;
            xP = uVShaft[i];
            xP /= mag(xP);
            yP = zP ^ xP;
            ...
            if (((xP >= xBlade - bladeProjectionRadius[i]) && (xP <= xBlade + bladeProjectionRadius[i])) &&
...
void horizontalAxisWindTurbinesALMAdvanced::findNacelleSearchCells(int turbineNumber);
{
    ...
    if (includeNacelle[i])
    {
        forAll(U_.mesh().cells(),cellI)
        {
            ...
            scalar r1 = 0.0;
            scalar r2 = 0.0;
            v1 = U_.mesh().cells()[cellI] - rotorApex[i];
            v2 = U_.mesh().cells()[cellI] - nacelleSamplePoint[i];
            zP.z() = 1.0;
            xP = uVShaft[i];
            xP /= mag(xP);
            yP = zP ^ xP;
