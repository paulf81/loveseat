...
{   
    ...
    for(int i = 0; i < numTurbinesDistinct; i++)
    {
        ...
        TorqueControllerType.append(word(turbineProperties.lookup("TorqueControllerType")));
        YawControllerType.append(word(turbineProperties.lookup("YawControllerType")));
        PitchControllerType.append(word(turbineProperties.lookup("PitchControllerType")));
        
        CutInGenSpeed.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("CutInGenSpeed")));
        RatedGenSpeed.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("RatedGenSpeed")));
        Region2StartGenSpeed.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("Region2StartGenSpeed")));
        Region2EndGenSpeed.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("Region2EndGenSpeed")));
        CutInGenTorque.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("CutInGenTorque")));
        RatedGenTorque.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("RatedGenTorque")));
        RateLimitGenTorque.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("RateLimitGenTorque")));
        KGen.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("KGen")));
        TorqueControllerRelax.append(readScalar(turbineProperties.subDict("TorqueControllerParams").lookup("TorqueControllerRelax")));

        PitchControlStartPitch.append(readScalar(turbineProperties.subDict("PitchControllerParams").lookup("PitchControlStartPitch")));
        PitchControlEndPitch.append(readScalar(turbineProperties.subDict("PitchControllerParams").lookup("PitchControlEndPitch")));
        PitchControlStartSpeed.append(readScalar(turbineProperties.subDict("PitchControllerParams").lookup("PitchControlStartSpeed")));
        PitchControlEndSpeed.append(readScalar(turbineProperties.subDict("PitchControllerParams").lookup("PitchControlEndSpeed")));
        RateLimitPitch.append(readScalar(turbineProperties.subDict("PitchControllerParams").lookup("RateLimitPitch")));
    ...
    for(int i = 0; i < numTurbines; i++)
    {
        ...
        //Info << projectionRadius/epsilon[i] << tab << sphereRadius << endl;
    }
    //Pout << turbinesControlled << endl;

    //List<vector> testList(numTurbines,vector::zero);


    //forAll(turbinesControlled,i)
    //{
    //    testList[i].z() = turbinesControlled[i];
    //    testList[i].x() = i*1.0845;
    //    testList[i].y() = i*456;
    //}
    //Info << testList << endl << endl;

    //Pstream::gather(testList,minOp<List<vector> >());
...
//             bladeRadius[i][k][m] = dist;
        ...
        // Define the size of the generator torque (not density normalized) lists and set to zero.
        torqueGen.append(0.0);
...
void horizontalAxisWindTurbinesALM::computeRotSpeed()
{
    ...
    {
        int j = turbineTypeID[i];
	if (TorqueControllerType[j] == "none")
        {
            // Compute the change in blade position at new rotor speed.
            deltaAzimuth[i] = rotSpeed[i] * dt;
        }
	else if (TorqueControllerType[j] == "fiveRegion")
        {
            // Get the generator speed.
            scalar genSpeed = (rotSpeed[i]/rpmRadSec)*GBRatio[j];

            // Save the generator torque from the last time step.
            scalar torqueGenOld = torqueGen[i];
            
            // Region 1.
            if (genSpeed < CutInGenSpeed[j])
            {
                //Info << "Region 1" << endl;
                torqueGen[i] = CutInGenTorque[j];
            }
            // Region 1-1/2.
            else if ((genSpeed >= CutInGenSpeed[j]) && (genSpeed < Region2StartGenSpeed[j]))
            {
                //Info << "Region 1-1/2" << endl;
                scalar dGenSpeed = genSpeed - CutInGenSpeed[j];
                ...
                torqueGen[i] = CutInGenTorque[j] + torqueSlope*dGenSpeed;
            }
            // Region 2.
            else if ((genSpeed >= Region2StartGenSpeed[j]) && (genSpeed < Region2EndGenSpeed[j]))
            {
                //Info << "Region 2" << endl;
                torqueGen[i] = KGen[j] * genSpeed * genSpeed;
            }
            // Region 2-1/2.
            else if ((genSpeed >= Region2EndGenSpeed[j]) && (genSpeed < RatedGenSpeed[j]))
            {
                //Info << "Region 2-1/2" << endl;
                scalar dGenSpeed = genSpeed - Region2EndGenSpeed[j];
                scalar Region2EndGenTorque = KGen[j] * Region2EndGenSpeed[j] * Region2EndGenSpeed[j];
                scalar torqueSlope = (RatedGenTorque[j] - Region2EndGenTorque) / ( RatedGenSpeed[j] - Region2EndGenSpeed[j] );
                torqueGen[i] = Region2EndGenTorque + torqueSlope*dGenSpeed;
            }
            // Region 3.
            else if (genSpeed >= RatedGenSpeed[j])
            {
                //Info << "Region 3" << endl;
                torqueGen[i] = RatedGenTorque[j];
            }

            // Limit the change in generator torque if after first time step (otherwise it slowly
            // ramps up from its zero initialized value--we want it to instantly be at its desired
            // value on the first time step, but smoothly vary from there).
            if ((Foam::mag((torqueGen[i] - torqueGenOld)/dt) > RateLimitGenTorque[j]) && (pastFirstTimeStep))
            {
                if (torqueGen[i] > torqueGenOld)
                {
                    torqueGen[i] = torqueGenOld + (RateLimitGenTorque[j] * dt);
                }
                else if (torqueGen[i] <= torqueGenOld)
                {
                    torqueGen[i] = torqueGenOld - (RateLimitGenTorque[j] * dt);
                }
            }
            ...
            rotSpeed[i] += TorqueControllerRelax[j]*(dt/DriveTrainIner[j])*(torqueRotor[i]*fluidDensity[i] - GBRatio[j]*torqueGen[i]);

            // Limit the rotor speed to be positive and such that the generator does not turn
            // faster than rated.
            rotSpeed[i] = max(0.0,rotSpeed[i]);
            rotSpeed[i] = min(rotSpeed[i],(RatedGenSpeed[j]*rpmRadSec)/GBRatio[j]);

            // Compute the change in blade position at new rotor speed.
            deltaAzimuth[i] = rotSpeed[i] * dt;

            //Info << "Turbine " << i << ":  " << pastFirstTimeStep << tab << "Generator Torque = " << torqueGen[i] * GBRatio[j] << tab <<
            //        "Aerodynamic Torque = " << torqueRotor[i]*fluidDensity[i] << tab << "Rotor Speed = " << rotSpeed[i]/rpmRadSec << endl;
	}
    }
}
        

void horizontalAxisWindTurbinesALM::computeNacYaw()
{
    // Proceed turbine by turbine.
...
void horizontalAxisWindTurbinesALM::computeBladePitch()
{
    // Proceed turbine by turbine.
    forAll(pitch, i)
    {
        ...
        else if (PitchControllerType[j] == "simpleLinear")
        {
            // Do a simple control that makes pitch linearly related
            // to rotor speed.  There is a low speed range in which
            // pitch is a fixed value.  There is a mid-speed range
            // in which pitch changes linearly.  There is a high-speed
            // range in which pitch is at another fixed value.

            // Save the blade pitch from the last time step.
            scalar pitchOld = pitch[i];

            // Compute the rotor speed of this turbine in RPM.
            scalar rotSpeedRPM = rotSpeed[i]/rpmRadSec;

            // If in the low-speed range, set the desired pitch to 
            // PitchControlStartPitch.
            if (rotSpeedRPM <= PitchControlStartSpeed[j])
            {
                pitch[i] = PitchControlStartPitch[j];
            }

            // If in the mid-speed range, apply the linear pitch-speed
            // relation.
            else if ((rotSpeedRPM > PitchControlStartSpeed[j]) && (rotSpeedRPM <= PitchControlEndSpeed[j]))
            {
                scalar pitchSlope = (PitchControlEndPitch[j] - PitchControlStartPitch[j])/(PitchControlEndSpeed[j] - PitchControlStartSpeed[j]);
                scalar deltaSpeed = rotSpeedRPM - PitchControlStartSpeed[j];
                pitch[i] = PitchControlStartPitch[j] + (pitchSlope * deltaSpeed);
            }

            // If in the high-speed range, set the desired pitch to
            // PitchControlEndPitch.
            else if (rotSpeedRPM > PitchControlEndSpeed[j])
            {
                pitch[i] = PitchControlEndPitch[j];
            }

            // Now that the desired pitch is known, adjust the current pitch at its control rate.
            if (Foam::mag((pitch[i] - pitchOld)/dt) > RateLimitPitch[j])
            {
                if (pitch[i] > pitchOld)
                {
                    pitch[i] = pitchOld + (RateLimitPitch[j] * dt);
                }
                else if (pitch[i] <= pitchOld)
                {
                    pitch[i] = pitchOld - (RateLimitPitch[j] * dt);
                }
            }
...
void horizontalAxisWindTurbinesALM::printDebug()
{
    ...
    Info << "TorqueControllerType = " << TorqueControllerType << endl;
