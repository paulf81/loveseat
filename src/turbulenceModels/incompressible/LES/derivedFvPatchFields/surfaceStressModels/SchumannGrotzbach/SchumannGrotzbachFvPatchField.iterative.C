...
)
{
    ...
    forAll(Rw, facei)
    {
        ...
        else if (averageType_ == "local")
        {
            RwP.xx() = 0.0;
            RwP.xy() = 0.0;
            RwP.xz() = -sqr(uStar[facei]) * (UParallelP[facei].x() / UParallelPMag[facei]);
            RwP.yy() = 0.0;
            RwP.yz() = -sqr(uStar[facei]) * (UParallelP[facei].y() / UParallelPMag[facei]);
            RwP.zz() = 0.0;
        }
            ...
            else if (qw < 0.0)
            {
                ...
                do
                {
                   ...
                   // Pout << "uStar0 = " << uStar0 << endl;
                    ...
                    uStar0 = max(0.0, (kappa * U) / (Foam::log(z1 / z0) - psiM0));

                    diff = uStar0 - uStarOld;
                ...
                } while (mag(diff) > tol && iter < iterMax);
